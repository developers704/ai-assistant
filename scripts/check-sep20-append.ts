/**
 * Post-append sanity: Sep 20 2026 rows land in the live sales seed.
 * Run: npx tsx scripts/check-sep20-append.ts
 *
...
const sep20 = rows.filter((r) => r.date === DAY);
assert.ok(sep20.length > 0, "Sep 20 rows must exist after append");

const netSales = sep20.reduce((s, r) => s + r.netRevenue, 0);
...
const splitCsv = sep20.filter(
  (r) => r.transactionId === "CL-10291107" && String(r.sku ?? r.itemNumber ?? "") === "227651V"
);
assert.equal(
  splitCsv.length,
  2,
  `CSV must keep POS split rows for CL-10291107 / 227651V, got ${splitCsv.length}`
);
...
assert.ok(
  Math.abs((merged[0]?.grossSales ?? 0) - 239) < 0.02,
  `merged Sales Amount ${merged[0]?.grossSales}`
);
assert.ok(
  Math.abs((merged[0]?.netRevenue ?? 0) - 179.68) < 0.02,
...
async function main() {
  ...
  assert.ok(
    Math.abs(netSales - 163093.22) < 0.02,
    `Sep 20 CSV Total expected 163093.22 got ${netSales}`
  );
  assert.equal(sep20.length, 389, `Sep 20 rows ${sep20.length}`);
  assert.equal(new Set(sep20.map((r) => r.storeName)).size, 30);
  ...
  assert.ok(Math.abs(paySum - 180022.07) < 1, `paycode sum ${paySum}`);
  assert.equal(byName["GE"], undefined);
  assert.equal(byName["AFRM"], undefined);

  console.log("check-sep20-append: ok", {
    sep20Rows: sep20.length,
    sep20NetSales: +netSales.toFixed(2),
    querySalesNet: +(q.summary!.netSales).toFixed(2),
    sep20Stores: new Set(sep20.map((r) => r.storeName)).size,
    dataThrough: dates.at(-1),
    paycodeSep20Applied: +paySum.toFixed(2),
