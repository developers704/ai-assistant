...
describe("bundled Payment-Transactions.csv", () => {
  ...
  it("Jan 1–Sep 22 2026 Applied Amt matches the full payment export", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    // Same export as the Excel Pay Code Group sheet. CASH and CC are short
    // by legs whose Transaction Date falls outside this window.
    ...
    expect(byName["KAFE"]).toBeCloseTo(5387815.48, 2);
    expect(byName["FLEX"]).toBeCloseTo(955769.02, 2);
    expect(byName["BREAD"]).toBeCloseTo(3437, 2);
    ...
    expect(sum).toBeCloseTo(59311274.59, 2);
    const all = paycodeTotalsForPaymentWindow({ legs }).reduce((s, t) => s + t.revenue, 0);
    expect(all).toBeCloseTo(59330897.55, 2);
    expect(all - sum).toBeCloseTo(19622.96, 2);
  });
});
