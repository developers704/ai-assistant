...
describe("bundled Payment-Transactions.csv", () => {
  ...
  it("Paycodes card Sep 5 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-05",
      to: "2026-09-05",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(142748.74, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(120021.1, 2);
    expect(byName["KAFE"]).toBeCloseTo(48895.27, 2);
    expect(byName["CASH"]).toBeCloseTo(25334.27, 2);
    expect(byName["SYNC"]).toBeCloseTo(25073.9, 2);
    expect(byName["BREAD"]).toBeCloseTo(5116.64, 2);
    expect(byName["PROG"]).toBeCloseTo(3467, 2);
    expect(byName["WELLS"]).toBeCloseTo(2795, 2);
    expect(byName["GAFCO"]).toBeCloseTo(2575, 2);
    expect(byName["AFFIRM"]).toBeCloseTo(1897.39, 2);
    expect(byName["ACIMA"]).toBeCloseTo(1875, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(379799.31, 2);
  });

  it("Paycodes card Sep 6 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-06",
      to: "2026-09-06",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["IDDEAL"]).toBeCloseTo(199647.03, 2);
    expect(byName["CC"]).toBeCloseTo(154549.43, 2);
    expect(byName["WELLS"]).toBeCloseTo(29954.98, 2);
    expect(byName["KAFE"]).toBeCloseTo(25345.15, 2);
    expect(byName["SYNC"]).toBeCloseTo(22424.53, 2);
  ...
  it("Paycodes card Sep 7 2026 matches the appended daily payment CSV", () => {
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["IDDEAL"]).toBeCloseTo(103149.89, 2);
    expect(byName["CC"]).toBeCloseTo(97915.02, 2);
    expect(byName["WELLS"]).toBeCloseTo(23438.98, 2);
    expect(byName["KAFE"]).toBeCloseTo(23369.19, 2);
    expect(byName["CASH"]).toBeCloseTo(20142.72, 2);
    expect(byName["SYNC"]).toBeCloseTo(16089.84, 2);
    expect(byName["FLEX"]).toBeCloseTo(5000, 2);
    expect(byName["ACIMA"]).toBeCloseTo(2614.89, 2);
    expect(byName["PROG"]).toBeCloseTo(1724.63, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
  ...
  it("Paycodes card Sep 9 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-09",
      to: "2026-09-09",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["IDDEAL"]).toBeCloseTo(72650.08, 2);
    expect(byName["CC"]).toBeCloseTo(61656.02, 2);
    expect(byName["WELLS"]).toBeCloseTo(40500, 2);
    expect(byName["CASH"]).toBeCloseTo(7329.7, 2);
    expect(byName["KAFE"]).toBeCloseTo(4918, 2);
    expect(byName["SYNC"]).toBeCloseTo(4282.4, 2);
    expect(byName["ACIMA"]).toBeCloseTo(2479.39, 2);
    expect(byName["PROG"]).toBeCloseTo(2300, 2);
    expect(byName["FLEX"]).toBeCloseTo(1210, 2);
    expect(byName["MULBRY"]).toBeCloseTo(76, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(197401.59, 2);
  });

  it("Paycodes card Sep 10 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-10",
      to: "2026-09-10",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["IDDEAL"]).toBeCloseTo(47702.46, 2);
    expect(byName["CC"]).toBeCloseTo(35415.88, 2);
    expect(byName["SYNC"]).toBeCloseTo(34507.07, 2);
    expect(byName["KAFE"]).toBeCloseTo(11044.67, 2);
    expect(byName["CASH"]).toBeCloseTo(10945.23, 2);
    expect(byName["ACIMA"]).toBeCloseTo(4803, 2);
    expect(byName["BREAD"]).toBeCloseTo(3100.26, 2);
    expect(byName["PROG"]).toBeCloseTo(1103.45, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(148622.02, 2);
  });

  it("Paycodes card Sep 12 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-12",
      to: "2026-09-12",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(181336.49, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(80674.14, 2);
    expect(byName["SYNC"]).toBeCloseTo(44180.06, 2);
    expect(byName["KAFE"]).toBeCloseTo(31775.89, 2);
    expect(byName["WELLS"]).toBeCloseTo(7175.57, 2);
    expect(byName["ACIMA"]).toBeCloseTo(6630.49, 2);
    expect(byName["CASH"]).toBeCloseTo(5710.89, 2);
    expect(byName["PROG"]).toBeCloseTo(2650, 2);
    expect(byName["AFFIRM"]).toBeCloseTo(400, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
  ...
  it("Paycodes card Sep 14 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-14",
      to: "2026-09-14",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["IDDEAL"]).toBeCloseTo(59738.28, 2);
    expect(byName["CC"]).toBeCloseTo(48060.55, 2);
    expect(byName["KAFE"]).toBeCloseTo(13406.99, 2);
    expect(byName["SYNC"]).toBeCloseTo(10131.84, 2);
    expect(byName["WELLS"]).toBeCloseTo(8000, 2);
    expect(byName["CASH"]).toBeCloseTo(4373.64, 2);
    expect(byName["ACIMA"]).toBeCloseTo(2420, 2);
    expect(byName["MULBRY"]).toBeCloseTo(60, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(146191.3, 2);
  });

  it("Paycodes card Sep 15 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-15",
      to: "2026-09-15",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(51943.78, 2);
    expect(byName["SYNC"]).toBeCloseTo(32524.67, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(16799.18, 2);
    expect(byName["CASH"]).toBeCloseTo(9850.36, 2);
    expect(byName["KAFE"]).toBeCloseTo(8329.08, 2);
    expect(byName["WELLS"]).toBeCloseTo(7072.97, 2);
    expect(byName["MULBRY"]).toBeCloseTo(20, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(126540.04, 2);
  });

  it("Paycodes card Sep 16 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-16",
      to: "2026-09-16",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(101645.77, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(53348.94, 2);
    expect(byName["SYNC"]).toBeCloseTo(12095.26, 2);
    expect(byName["KAFE"]).toBeCloseTo(6152.99, 2);
    expect(byName["CASH"]).toBeCloseTo(2050.68, 2);
    expect(byName["ACIMA"]).toBeCloseTo(890, 2);
    expect(byName["MULBRY"]).toBeCloseTo(48, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(176231.64, 2);
  });

  it("Paycodes card Sep 17 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-17",
      to: "2026-09-17",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CASH"]).toBeCloseTo(84334.7, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(53338.15, 2);
    expect(byName["CC"]).toBeCloseTo(38186.45, 2);
    expect(byName["SYNC"]).toBeCloseTo(16136.29, 2);
    expect(byName["KAFE"]).toBeCloseTo(15207.25, 2);
    expect(byName["ACIMA"]).toBeCloseTo(3839, 2);
    expect(byName["PROG"]).toBeCloseTo(2000, 2);
    expect(byName["AFFIRM"]).toBeCloseTo(650, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(213691.84, 2);
  });

  it("Paycodes card Sep 18 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-18",
      to: "2026-09-18",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(65796.27, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(41858.82, 2);
    expect(byName["SYNC"]).toBeCloseTo(30300.88, 2);
    expect(byName["KAFE"]).toBeCloseTo(23588.64, 2);
    expect(byName["CASH"]).toBeCloseTo(13154.67, 2);
    expect(byName["PROG"]).toBeCloseTo(5400, 2);
    expect(byName["ACIMA"]).toBeCloseTo(3277.5, 2);
    expect(byName["WELLS"]).toBeCloseTo(2822.09, 2);
    expect(byName["FLEX"]).toBeCloseTo(1650, 2);
    expect(byName["GE"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
  ...
  it("Paycodes card Sep 20 2026 matches the appended daily payment CSV", () => {
    const file = path.join(process.cwd(), "data/reports/Payment-Transactions.csv");
    const totals = paycodeTotalsForPaymentWindow({
      from: "2026-09-20",
      to: "2026-09-20",
    ...
    const byName = Object.fromEntries(totals.map((t) => [t.name, t.revenue]));
    expect(byName["CC"]).toBeCloseTo(62628.89, 2);
    expect(byName["IDDEAL"]).toBeCloseTo(52457.26, 2);
    expect(byName["KAFE"]).toBeCloseTo(14879, 2);
    expect(byName["SYNC"]).toBeCloseTo(14447, 2);
    expect(byName["FLEX"]).toBeCloseTo(12965.24, 2);
    expect(byName["CASH"]).toBeCloseTo(11833.06, 2);
    expect(byName["WELLS"]).toBeCloseTo(5999, 2);
    expect(byName["AFFIRM"]).toBeCloseTo(3878.62, 2);
    expect(byName["PROG"]).toBeCloseTo(770, 2);
    expect(byName["MULBRY"]).toBeCloseTo(164, 2);
    expect(byName["GE"]).toBeUndefined();
    expect(byName["AFRM"]).toBeUndefined();
    const sum = totals.reduce((s, t) => s + t.revenue, 0);
    expect(sum).toBeCloseTo(180022.07, 2);
  });
});
