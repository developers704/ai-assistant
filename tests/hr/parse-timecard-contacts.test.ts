...
describe("timecard contact columns", () => {
  it("keeps UserEmail for chat and Email for outbound mail", () => {
    const [row] = parseTimecardCsv(
      "Payroll Name,Designation,In Date,Time In,Time Out,User Email,Email\n" +
        '"Doe, Jane",Sales Associate,8/4/2026,9:00 AM,5:00 PM,chat@example.com,mail@example.com\n'
    );
  ...
  it("accepts punctuation and spacing in contact headers", () => {
    const [row] = parseTimecardCsv(
      "Payroll Name,Designation,In Date,Time In,Time Out,User-Email,Employee Email\n" +
        '"Doe, Jane",Sales Associate,8/4/2026,9:00 AM,5:00 PM,chat@example.com,mail@example.com\n'
    );
