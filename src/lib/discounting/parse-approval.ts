...
function looksLikeApproverCode(tok: string): boolean {
  ...
  if (
    /^(WELLS|WELLSFARGO|IDDEAL|IDEAL|IDEA|SYNCHRONY|SYNC|SYCHY|FLEX|FLEXPAY|PROG|PROGRESSIVE|ACIMA|ACIM|UOWN|KAFENE|KAFE|AFFIRM|AFF|CASH|CC|FINANCE|GE)$/i.test(
      t
    )
  ) {
    return false;
