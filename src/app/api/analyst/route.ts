...
7b. DATE FILTER SQL (critical for Valliani CSVs): Transaction dates are often stored as US m/d/Y text (e.g. 7/10/2026 = 10 July 2026). Always filter with a robust expression, never a bare string match:
   coalesce(try_cast("Transaction Date" AS DATE), try_strptime(trim(cast("Transaction Date" AS VARCHAR)), '%m/%d/%Y')::DATE, try_strptime(trim(cast("Transaction Date" AS VARCHAR)), '%d/%m/%Y')::DATE) = DATE '2026-07-10'
   Do NOT use "Transaction Date" = '10/07/2026' or = '7/10/2026'. Do NOT interpret "10 July" as October.
7c. DESIGN FILTER: use lower(trim("Design")) = 'novello' (or IN list from allValues). Never rely on case-sensitive "Design" = 'Novello'.
7d. For a single total, use COALESCE(SUM("Total"), 0) AS total_sales so missing matches return 0 not NULL.
...
17. STORE SALES REPORT — when columns include "Transaction #", "Item #" or "SKU #", "Style #", "Description", "Vendor Name" or "VendorModel1", "Store", "Department", "Design", "Class", "Sub-Class", "Qty", "Inventory Cost", "Sales Amount", "Disc Amt", "Total": use "Total" as net revenue, "Sales Amount" as gross, "Disc Amt" for discounts, "Qty" for units. Do not compute or report estimated margin unless the user explicitly asks for cost or profit. "Item #" / "SKU #" identifies products; "Vendor Name" is the supplier; "Class" is metal type (14KT, 10KT, etc.); "Sub-Class" is style detail. Group by "Store", "Department", "Design", "Vendor Name", "Class", or "Transaction Date" when present. Count distinct "Transaction #" for transaction counts.
