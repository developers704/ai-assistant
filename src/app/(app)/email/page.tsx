    ...
    <div className="flex flex-col h-[calc(100dvh-5.5rem)] lg:h-[calc(100dvh-4rem)]">
      <div className="glass-panel-strong rounded-3xl flex flex-col flex-1 min-h-0 overflow-hidden ring-1 ring-white/10">
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4 space-y-4 min-h-0">
          {!googleConnected && (
            <Card className="p-4 flex flex-wrap items-center justify-between gap-3 ring-1 ring-amber-400/20 bg-amber-500/10">
          ...
          {(assistantBusy || assistantMessage) && (
            <Card className="p-4 ring-1 ring-violet-400/20">
              ...
              {assistantBusy ? (
                <p className="text-sm text-ink-muted flex items-center gap-2">
                  <Loader2 size={16} className="animate-spin text-violet-300" /> Working on your request…
                </p>
              ) : assistantMessage ? (
                <ChatBubble
          ...
          {loading && googleConnected && emails.length === 0 ? (
            <Card className="p-10 text-center text-ink-muted text-sm flex flex-col items-center gap-3">
              <Loader2 size={24} className="animate-spin text-blue-300" />
              Loading Gmail inbox…
            </Card>
          ) : emails.length === 0 ? (
            <Card className="p-12 text-center">
              <Mail size={40} className="text-ink-muted mx-auto mb-3" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 lg:gap-5 lg:min-h-[420px]">
              <div className="lg:col-span-2 space-y-2 lg:overflow-y-auto lg:max-h-[calc(100dvh-18rem)] pr-0.5">
                <p className="text-xs text-ink-muted px-1 mb-1 sticky top-0 py-1 backdrop-blur-sm">
                ...
                {emails.map((email) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className="text-[11px] text-ink-muted flex-shrink-0">{formatRelativeTime(email.receivedAt)}</span>
                    </div>
              ...
              <div className="lg:col-span-3 lg:min-h-[320px]">
                {selected ? (
                  <Card className="h-full overflow-y-auto lg:max-h-[calc(100dvh-18rem)]">
                    ...
                    <div className="flex gap-2 border-t border-white/10 pt-4 flex-wrap">
                      ...
                      {selected.needsReply && (
                        <Button size="sm" variant="outline" disabled={assistantBusy} onClick={() => handleFollowUp(selected)}>
                  ...
                  <Card className="h-full min-h-[280px] flex flex-col items-center justify-center text-center">
